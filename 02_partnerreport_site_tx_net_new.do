...
*import/open data
	capture confirm file "$output\ICPIFactView_ALLTX_Site_IM${datestamp}.dta"
		if !_rc{
			use "$output\ICPIFactView_ALLTX_Site_IM${datestamp}.dta", clear
		}
		else{
			import delimited "$data\Site_IM_${datestamp}_ALLTX.txt", clear
			save "$output\ICPIFactView_ALLTX_Site_IM${datestamp}.dta", replace
		}
	*end
...
*TX_NET_NEW indicator
	...
	*create copy periods to replace "." w/ 0 for generating net new (if . using in calc --> answer == .)
	foreach x in fy2015q4 fy2016q4 fy2016_targets{
		clonevar `x'_cc = `x'
		recode `x'_cc (. = 0)
		}
		*end
...
*create retained on TX variable (TX_NET_NEW - TX_NEW) --> dataset needs to be wide
	...
	*reshape wide
		...
		*egen id = group(psnuuid mechanismid primepartner communityuid facilityuid indicatortype) 
		reshape wide fy2015q2 fy2015apr fy2016_targets fy2016apr, ///
			...
			j(indicator, string)
	*create copy periods to replace "." w/ 0 for generating net new (if . using in calc --> answer == .)
		foreach fy in fy2016aprTX_NEW fy2016aprTX_NET_NEW{
	...
	*new variable - not retained on treatment (will be stored in TX_CURR)
		*gen tx_not_ret = fy2016aprTX_NEW_cc-fy2016aprTX_NET_NEW_cc
		gen tx_ret = fy2016aprTX_NET_NEW_cc - fy2016aprTX_NEW_cc
			*change to missing if not in both time periods  
			replace tx_ret = . if ///
				(inlist(fy2016aprTX_NEW, ., 0) & inlist(fy2016aprTX_NET_NEW, ., 0)) | /// if either TX_NEW and NET_NEW were missing
				(inlist(fy2015aprTX_CURR, ., 0) & inlist(fy2016aprTX_CURR, ., 0))
		drop *_cc
...
* delete extrainous vars/obs
	rename tx_ret fy2016apr_tx_ret 
	drop if indicator=="TX_NET_NEW" // only needed for not retain calculation
	replace fy2016apr_tx_ret = . if indicator!="TX_CURR" //only want one obs of tx_retained
	drop if fy2015apr==. & fy2016apr==. & fy2016apr_tx_ret==. //drop if all relevant pd results are both missing
...
*save as dta file for appending onto PSNU version [need to add to 02 do file]
	save "$output\ICPIFactView_SiteIM_${date}_ALLTX", replace
